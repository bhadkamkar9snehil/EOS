(() => {
  ...
  function ensureHeaderBank() {
    ...
    bank.innerHTML = `
      <button type="button" class="rcb-month rcb-calendar" aria-label="Reporting month"><span class="rcb-icon">▣</span><strong class="rcb-month-label">Month</strong><span class="rcb-chevron">⌄</span></button>
      ...
      <button type="button" class="rcb-rotary" aria-label="Search and filter controls"><i></i></button>`;
    const actions = qs('.topbar-actions', topbar);
    topbar.insertBefore(bank, actions || null);

    const input = qs('.rcb-search input', bank);
    input?.addEventListener('focus', () => click('.global-search .icon-button'));
    input?.addEventListener('input', e => {
      const hidden = qs('.global-search-field input');
    ...
    qs('.rcb-rotary', bank)?.addEventListener('click', () => {
      const target = qs('.rcb-search input', bank);
      target?.focus();
    });
    qs('[data-rcb="team"]', bank)?.addEventListener('click', () => showHint('Team scope is currently All.'));
    qs('[data-rcb="group"]', bank)?.addEventListener('click', () => showHint('Group scope is currently All.'));
    qs('.rcb-month', bank)?.addEventListener('click', () => click('.month-control button:last-child'));
    syncHeaderBank();
  }
  ...
  function ensureLowerConsole() {
    ...
    panel.innerHTML = `
      ...
      <button class="realist-dial" type="button" aria-label="Interface style control"><i></i></button>`;
    ...
    qs('.realist-dial', panel)?.addEventListener('click', () => {
      if (window.epaSkin?.apply) window.epaSkin.apply(isRealist() ? 'minimal' : 'realist');
    });
  }
