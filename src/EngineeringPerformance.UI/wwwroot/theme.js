(() => {
    ...
    const palettes = {
        ...
        dracula:{name:'Amethyst Night',desc:'Purple-led low-light palette',category:'professional-dark',dark:true,ink:'#f8f8f2',text2:'#c2c4d6',muted:'#9a9cb5',border:'#50536a',bg:'#282a36',surface:'#3b3e50',surface2:'#303341',accent:'#bd93f9',header:'#1e1f29'},
        gruvbox:{name:'Copper Night',desc:'Warm low-light contrast',category:'editor-inspired',dark:true,ink:'#f2e6c5',text2:'#c9bca0',muted:'#a39582',border:'#5b524c',bg:'#282828',surface:'#393533',surface2:'#302e2d',accent:'#fe8019',header:'#1d2021'},
        monokai:{name:'Magenta Night',desc:'Expressive dark palette',category:'editor-inspired',dark:true,ink:'#f8f8f2',text2:'#c8c9be',muted:'#9b9c94',border:'#525249',bg:'#272822',surface:'#3a3b32',surface2:'#303129',accent:'#f92672',header:'#1e1f1a'},
        everforest:{name:'Forest Night',desc:'Soft green low-light palette',category:'professional-dark',dark:true,ink:'#e0d4bc',text2:'#b2bbb2',muted:'#90998d',border:'#566168',bg:'#2d353b',surface:'#3c484d',surface2:'#343e43',accent:'#a7c080',header:'#232a2e'},
        'solarized-dark':{name:'Solar Night',desc:'Amber-led analytical dark',category:'professional-dark',dark:true,ink:'#f2ecd9',text2:'#a7b4b4',muted:'#7f949b',border:'#1a5561',bg:'#002b36',surface:'#0b414e',surface2:'#063440',accent:'#c39a16',header:'#00212b'},
        'contrast-dark':{name:'High Contrast Dark',desc:'Maximum low-light legibility',category:'high-contrast',dark:true,ink:'#ffffff',text2:'#e6e6e6',muted:'#b8b8b8',border:'#777777',bg:'#000000',surface:'#181818',surface2:'#0c0c0c',accent:'#8ab4ff',header:'#000000'}
    };

    const semantic = {
        light:{good:'#16794A',warning:'#946200',serious:'#B45309',critical:'#B42318',info:'#0F6E9E',missing:'#667085'},
        dark:{good:'#67D391',warning:'#F0C967',serious:'#F3A56B',critical:'#FF8178',info:'#82BCE4',missing:'#A2AAB7'}
    };
    ...
    const categorical = (palette, intensity) => {
        const base = toOklch(palette.accent);
        const factor = intensityScale[intensity] || 1;
        const offsets = [0, 52, 118, 188, 252, 308, 82, 222];
        const lightness = palette.dark ? [.72,.76,.74,.78,.73,.77,.70,.75] : [.48,.50,.46,.52,.47,.51,.44,.49];
        const baseC = Math.max(.075, Math.min(.145, base[1] || .1)) * factor;
        return offsets.map((offset, i) => toHex([lightness[i], Math.max(.055, Math.min(.18, baseC * (i % 3 === 1 ? .92 : 1))), (base[2] + offset) % 360]));
    };
    const heatScale = (accent, dark, intensity) => {
        const [,C,H] = toOklch(accent);
        const factor = intensityScale[intensity] || 1;
        const levels = dark ? [.25,.37,.50,.63,.76] : [.94,.82,.68,.53,.39];
        return levels.map((L, i) => toHex([L, Math.max(.035, Math.min(.14, C * factor * (.75 + i * .07))), H]));
    };
    const focusColor = (accent, surfaces, dark) => {
        const candidates = [accent, dark ? '#9CC4FF' : '#174EA6', dark ? '#FFD166' : '#7A2E00'];
    ...
    const installTokens = palette => {
        ...
        const sem = isDark ? semantic.dark : semantic.light;
        const series = categorical(palette, strength);
        const heat = heatScale(palette.accent, isDark, strength);

        const canvas = palette.bg;
        const inset = mix(palette.bg, palette.surface2, .58);
        const base = palette.surface2;
        const soft = mix(palette.surface2, palette.surface, .34);
        const card = mix(palette.surface2, palette.surface, .68);
        const raised = palette.surface;
        const selected = mix(card, palette.accent, isDark ? .11 : .075);
        const focus = focusColor(palette.accent, [canvas, card, raised, selected], isDark);
        ...
        const tokens = {
            ...
            '--line':palette.border,'--line-soft':mix(palette.border, raised, isDark ? .34 : .58),'--line-strong':mix(palette.border, palette.text2, .28),
            ...
            '--secondary':series[1],'--secondary-hover':shiftLightness(series[1],isDark?.06:-.06),'--secondary-dark':isDark?shiftLightness(series[1],.08):shiftLightness(series[1],-.1),'--secondary-soft':mix(card,series[1],isDark?.14:.09),'--on-secondary':foregroundFor(series[1]),'--secondary-ink':foregroundFor(series[1]),
            ...
            '--nav-bg':palette.header,'--nav-line':mix(palette.header,foregroundFor(palette.header),.12),'--nav-text':mix(palette.header,foregroundFor(palette.header),.80),'--nav-text-strong':foregroundFor(palette.header),'--nav-muted':mix(palette.header,foregroundFor(palette.header),.60),'--nav-icon':mix(palette.header,foregroundFor(palette.header),.70),'--nav-hover':`color-mix(in srgb, ${palette.accent} ${Math.max(12,selectionStrength-6)}%, transparent)`,'--nav-selected':`color-mix(in srgb, ${palette.accent} ${selectionStrength}%, transparent)`,
            '--derived':derived,'--derived-soft':mix(card,derived,isDark?.12:.07),'--on-derived':foregroundFor(derived),
            '--shadow-resting':isDark?'0 1px 0 rgba(255,255,255,.025) inset':'none','--shadow-raised':isDark?'0 10px 28px rgba(0,0,0,.46),0 1px 0 rgba(255,255,255,.05) inset':'0 8px 22px rgba(20,28,40,.12)','--shadow-drawer':isDark?'0 28px 70px rgba(0,0,0,.66)':'0 28px 70px rgba(20,28,40,.28)','--shadow-nav':isDark?'1px 0 0 rgba(255,255,255,.04) inset':'none',
            '--chart-grid':mix(palette.border,card,isDark?.45:.64),'--chart-axis':mix(palette.border,palette.text2,.28),'--chart-muted':palette.muted,'--chart-ink':palette.ink,'--chart-tooltip':palette.header,'--on-chart-tooltip':foregroundFor(palette.header),'--chart-tooltip-text':foregroundFor(palette.header),'--chart-surface':raised,
        ...
        tokens['--chart-heat-low'] = heat[0];
        tokens['--chart-heat-high'] = heat[heat.length - 1];
    ...
    const chartPalette = () => ({
        ...
        heatScale:Array.from({length:5},(_,i)=>cssValue(`--chart-heat-${i+1}`,'#2563EB')),
    ...
    const diagnostics = value => {
        const key = normalizeTheme(value), p = palettes[key], strength = intensity(), s = p.dark ? semantic.dark : semantic.light;
        const card = mix(p.surface2,p.surface,.68), focus = focusColor(p.accent,[p.bg,card,p.surface],p.dark), series = categorical(p,strength);
        return { dark:p.dark, bodyContrast:contrast(p.ink,card), mutedContrast:contrast(p.muted,card), focusContrast:Math.min(contrast(focus,p.bg),contrast(focus,card),contrast(focus,p.surface)), categoricalDistinct:series.length===new Set(series).size };
    };

    window.epaTheme = {
        get:() => normalizeTheme(document.documentElement.dataset.theme || effectiveTheme()),
        selected:selectedTheme,
        apply,
        chartPalette,
        palettes:() => palettes,
        mode:{get:mode,apply:applyMode},
        intensity:{get:intensity,apply:applyIntensity},
        density:{get:density,apply:applyDensity},
        motion:{get:motion,apply:applyMotion,isReduced:isReducedMotion},
        diagnostics,
        initialize:() => {
            document.documentElement.dataset.density=density();
            document.documentElement.dataset.motion=motion();
            document.documentElement.dataset.themeMode=mode();
            return install(effectiveTheme(),false);
        }
    };
