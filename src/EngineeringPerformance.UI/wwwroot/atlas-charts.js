...
(() => {
    ...
    function sparkline(id,values,role='operational'){
        ...
        const draw=(silent=false)=>{
            ...
            const lastIndex=(()=>{for(let i=data.length-1;i>=0;i--)if(data[i]!=null)return i;return-1;})();
            const last=lastIndex>=0?[[lastIndex,data[lastIndex]]]:[];
            chart.setOption({
                ...(silent?{animation:false}:motion()),
                grid:{left:2,right:4,top:4,bottom:4},
                xAxis:{type:'category',show:false,data:data.map((_,i)=>i),boundaryGap:false},
                yAxis:{type:'value',show:false,min:0,max:100},
                tooltip:{show:false},
                series:[
                    {type:'line',data,symbol:'none',connectNulls:true,smooth:.24,lineStyle:{color,width:realist()?2.35:2},areaStyle:{color,opacity:realist()?.09:.045},markLine:{silent:true,symbol:'none',label:{show:false},lineStyle:{color:p.axis,width:1,opacity:realist()?.32:.16},data:[{yAxis:50}]}},
                    {type:'scatter',silent:true,data:last,symbolSize:realist()?6:5,itemStyle:{color,borderColor:p.surface,borderWidth:1.4}}
                ]
            },true);
        };
    ...
    function performanceField(id,points,xMax,utilizationTarget,medianX,selectedName){
        ...
        const draw=(silent=false)=>{
            ...
            const movers=source.filter(x=>x.prevX>0&&x.prevY>0&&!x.missing);
            const tails=movers.map((pt,i)=>{
                const isSelected=String(pt.name).toLowerCase()===selected;
                return {name:`move-${i}`,type:'line',silent:true,symbol:'none',data:[[pt.prevX,pt.prevY],[pt.x,pt.y]],lineStyle:{color:isSelected?orange():p.inkSoft,width:isSelected?2.4:physical?1.25:1.05,opacity:isSelected?.96:physical?.48:.34},z:1};
            });
            const previousData=movers.map(pt=>({name:pt.name,value:[pt.prevX,pt.prevY],selected:String(pt.name).toLowerCase()===selected}));
            const rawMax=Math.max(1,+xMax||Math.max(...source.map(x=>x.x),1)*1.12);
            const interval=rawMax>260?50:rawMax>120?25:10;
            const maxX=Math.max(20,Math.ceil(rawMax/interval)*interval),target=+utilizationTarget||75,med=+medianX||maxX/2;
            const scatterData=source.map(d=>{
                ...
                return {...d,value:[d.x,d.y],itemStyle:{
                    ...
                    shadowBlur:isSelected?(physical?18:14):physical?2:0,
                    shadowColor:isSelected?'rgba(242,106,18,.34)':'rgba(0,0,0,.16)'
                }};
            });
            const zoneAlpha=physical?.035:.018;
            chart.setOption({
                ...(silent?{animation:false}:motion()),
                grid:{left:physical?66:56,right:physical?30:24,top:physical?40:34,bottom:physical?62:56},
                ...
                xAxis:{type:'value',min:0,max:maxX,splitNumber:6,name:'ACCOUNTABLE / PUNCH HOURS',nameLocation:'middle',nameGap:physical?39:34,nameTextStyle:{color:p.inkSoft,fontSize:physical?12:11.5,fontWeight:650},axisLine:{show:true,lineStyle:{color:p.inkSoft,width:physical?1.2:1,opacity:.72}},axisTick:{show:true,length:physical?5:3,lineStyle:{color:p.axis}},axisLabel:{color:p.inkSoft,fontSize:12,margin:10},splitLine:{show:true,lineStyle:{color:p.grid,width:1,type:physical?'solid':'dashed',opacity:physical?.6:.42}}},
                yAxis:{type:'value',min:0,max:110,splitNumber:5,name:'UTILIZATION (%)',nameLocation:'end',nameGap:physical?18:14,nameTextStyle:{color:p.inkSoft,fontSize:physical?12:11.5,fontWeight:650,align:'left'},axisLine:{show:true,lineStyle:{color:p.inkSoft,width:physical?1.2:1,opacity:.72}},axisTick:{show:true,length:physical?5:3,lineStyle:{color:p.axis}},axisLabel:{color:p.inkSoft,fontSize:12,margin:10},splitLine:{show:true,lineStyle:{color:p.grid,width:1,type:physical?'solid':'dashed',opacity:physical?.6:.42}}},
                series:[
                    {name:'zones',type:'line',silent:true,symbol:'none',data:[],markArea:{silent:true,label:{color:p.inkSoft,fontSize:physical?11.5:11,fontWeight:physical?650:500},itemStyle:{borderWidth:0},data:[
                        [{name:'UNDERUSED',xAxis:0,yAxis:target,itemStyle:{color:`rgba(15,95,122,${zoneAlpha})`}},{xAxis:med,yAxis:110}],
                        [{name:'OVERLOADED',xAxis:med,yAxis:target,itemStyle:{color:`rgba(242,106,18,${zoneAlpha})`}},{xAxis:maxX,yAxis:110}],
                        [{name:'INCONSISTENT',xAxis:0,yAxis:0,itemStyle:{color:`rgba(180,35,24,${zoneAlpha})`}},{xAxis:med,yAxis:target}],
                        [{name:'BALANCED',xAxis:med,yAxis:0,itemStyle:{color:`rgba(15,95,122,${zoneAlpha})`}},{xAxis:maxX,yAxis:target}]
                    ]},markLine:{silent:true,symbol:'none',label:{show:false},lineStyle:{color:p.inkSoft,type:'dashed',width:physical?1.25:1,opacity:physical?.68:.52},data:[{xAxis:med},{yAxis:target}]}},
                    ...tails,
                    {name:'Previous position',type:'scatter',silent:true,z:2,data:previousData,symbol:'circle',symbolSize:physical?9:7,itemStyle:{color:p.surface,borderColor:p.inkSoft,borderWidth:physical?1.5:1.2,opacity:physical?.82:.68}},
                    {name:'Engineers',type:'scatter',z:4,data:scatterData,symbol:'circle',symbolSize:(value,params)=>Math.max(24,Math.min(50,19+(+params.data.score||0)*.29)),cursor:'pointer',
                        label:{show:true,formatter:x=>x.data.missing?`{missing|${initials(x.data.name)}}`:`{normal|${initials(x.data.name)}}`,fontWeight:700,fontSize:physical?11.5:11,rich:{normal:{color:'#fff',fontWeight:700},missing:{color:p.inkSoft,fontWeight:700}}},
    ...
    function movementRiver(id,categories,series,median,selectedName){
        ...
        const draw=(silent=false)=>{
            ...
            const rendered=rows.map((s,i)=>{
                const isSelected=String(s.name||'').toLowerCase()===selected,isAttention=!!s.attention,color=isSelected?orange():isAttention?p.operational:p.axis;
                return{name:s.name,type:'line',data:s.values||[],smooth:.24,connectNulls:true,showSymbol:false,symbol:'circle',lineStyle:{color,width:isSelected?2.7:isAttention?physical?1.55:1.4:physical?.85:.8,opacity:isSelected?1:isAttention?physical?.76:.68:physical?.19:.16},itemStyle:{color},emphasis:{focus:'series',lineStyle:{width:2.6,opacity:1}},endLabel:{show:isSelected||isAttention,formatter:x=>x.value==null?'':`${initials(s.name)}  ${(+x.value).toFixed(0)}`,color,fontSize:physical?11.5:11,fontWeight:isSelected?700:600,distance:6},labelLayout:{moveOverlap:'shiftY'},z:isSelected?5:isAttention?3:1};
            });
            rendered.push({name:'Team median',type:'line',data:median||[],smooth:.24,showSymbol:false,silent:true,lineStyle:{color:p.ink,width:physical?1.7:1.5,type:'dashed',opacity:.74},z:2});
            chart.setOption({...(silent?{animation:false}:motion()),grid:{left:physical?48:44,right:physical?88:82,top:physical?22:18,bottom:physical?44:42},tooltip:{...tip(p),trigger:'axis',formatter:params=>`<strong>${params[0]?.axisValueLabel||''}</strong><br/>${params.filter(x=>x.value!=null).sort((a,b)=>b.value-a.value).slice(0,8).map(x=>`${x.marker}${x.seriesName}: <b>${(+x.value).toFixed(1)}</b>`).join('<br/>')}`},xAxis:{type:'category',boundaryGap:false,data:categories||[],axisLine:{lineStyle:{color:p.inkSoft,opacity:.65}},axisTick:{show:physical},axisLabel:{color:p.inkSoft,fontSize:11.5,margin:10}},yAxis:{type:'value',min:0,max:100,splitNumber:4,axisLine:{show:physical,lineStyle:{color:p.inkSoft,opacity:.55}},axisTick:{show:false},axisLabel:{color:p.inkSoft,fontSize:11.5},splitLine:{lineStyle:{color:p.grid,opacity:physical?.58:.4}}},series:rendered},true);
        };
        draw();renderers.set(id,()=>draw(true));
    }
    ...
    window.addEventListener('epa-theme-changed',refresh);window.addEventListener('epa-motion-changed',refresh);window.addEventListener('epa-skin-changed',refresh);
    window.epaAtlas={
        registerDrilldown,
