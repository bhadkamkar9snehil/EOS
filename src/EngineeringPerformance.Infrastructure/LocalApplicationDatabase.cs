        ...
        if (excluded)
        {
            ...
            else
            {
                ...
                var canonicalName = matches
                    ...
                    .First();
                var canonical = matches.FirstOrDefault(x =>
                    string.Equals(x.EmployeeName, canonicalName, StringComparison.Ordinal));

                if (canonical is not null)
                {
                    var duplicates = matches.Where(x => !ReferenceEquals(x, canonical)).ToArray();
                    if (duplicates.Length > 0) context.AnalysisExclusions.RemoveRange(duplicates);
                }
                else
                {
