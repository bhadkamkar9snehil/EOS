...
function Stop-EpaProcesses {
    ...
    Get-Process -Name 'EngineeringPerformance.DesktopHost' -ErrorAction SilentlyContinue |
    ...
    try {
        ...
        Write-Log "Process scan warning: $($_.Exception.Message)"
    }
...
try {
    ...
    Invoke-Native -FilePath 'git' -Arguments @('fetch', 'origin', 'main') -Stage 'SYNC' -Message 'Fetching the latest EPA from GitHub…'
    Invoke-Native -FilePath 'git' -Arguments @('switch', 'main') -Stage 'SYNC' -Message 'Switching to the production main branch…'
    Invoke-Native -FilePath 'git' -Arguments @('pull', '--ff-only', 'origin', 'main') -Stage 'SYNC' -Message 'Updating EPA to the latest production version…'

    Invoke-Native -FilePath 'dotnet' -Arguments @('clean', 'EngineeringPerformance.slnx', '-c', 'Release') -Stage 'CLEAN' -Message 'Cleaning the previous build…'
    Invoke-Native -FilePath 'dotnet' -Arguments @('restore', 'EngineeringPerformance.slnx') -Stage 'RESTORE' -Message 'Restoring EPA dependencies…'
    Invoke-Native -FilePath 'dotnet' -Arguments @('build', 'EngineeringPerformance.slnx', '-c', 'Release', '--no-restore') -Stage 'BUILD' -Message 'Building EPA…'
    Invoke-Native -FilePath 'dotnet' -Arguments @('test', 'EngineeringPerformance.slnx', '-c', 'Release', '--no-build') -Stage 'TEST' -Message 'Running EPA validation tests…'
    ...
    if ($null -eq $exe) {
        throw 'EPA executable was not produced by the build.'
    }
